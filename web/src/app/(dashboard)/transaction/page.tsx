...
export default function TransactionsPage() {
  ...
  const fetchList = async () => {
    setLoadingList(true);
    try {
      const listData = await transactionService.getTransactions(filters);
  ...
  const fetchStats = async () => {
    ...
    try {
      const { search, ...statsFilters } = filters;
      const statsData = await transactionService.getStats(statsFilters);
      setStats(statsData);
    } catch (error) {
      console.error("Failed to fetch transactions stats:", error);
    } finally {
      setLoadingStats(false);
    }
  };

  // Debounce search effect for list
  useEffect(() => {
  ...
  // Fetch stats only when non-search filters change
  useEffect(() => {
    fetchStats();
    ...
    <div className="flex flex-col h-full gap-6">
      ...
      <div className="flex flex-col flex-1 bg-white/50 dark:bg-app-surface rounded-3xl p-6 shadow-sm border border-gray-100 dark:border-white/5 min-h-[500px]">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-6">
          {/* Tabs / Type Filter */}
          <div className="flex p-1 bg-gray-100 dark:bg-black/20 rounded-xl overflow-x-auto w-full lg:w-auto">
          ...
          <div className="flex flex-col sm:flex-row w-full lg:w-auto items-center gap-3">
            <div className="flex items-center gap-2 w-full sm:w-auto">
              <input
                ...
                className="w-full sm:w-auto px-3 py-2.5 rounded-xl border border-gray-200 dark:border-white/10 dark:bg-black/20 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 text-sm transition-all"
              />
            ...
            <input
              ...
              className="w-full sm:w-64 px-4 py-2.5 rounded-xl border border-gray-200 dark:border-white/10 dark:bg-black/20 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 text-sm transition-all"
            />
          </div>
        </div>
